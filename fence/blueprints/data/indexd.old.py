...
from azure.storage.blob import AccessPolicy, ResourceTypes, ContainerSasPermissions, generate_account_sas, generate_container_sas,  AccountSasPermissions
...
class BlankIndex(object):
    ...
    def make_signed_url(self, file_name, expires_in=None):
        ...
        if bucket in flag:
            ...
            url = AzIndexedFileLocation(az_url).make_azure_signed_url(az_secret_access_key, bucket, self.guid, self.file_name)  
        else:
...
class IndexedFile(object):
    ...
    def _get_signed_url(
        ...
        if not protocol:
            ...
            try:
                print(" self indexed file location zero :{}".format(self.indexed_file_locations[0]))
...
class AzIndexedFileLocation(IndexedFileLocation):
    ...
    def get_signed_url(
        ...
        url = AzIndexedFileLocation.make_azure_signed_url(
                az_secret_access_key, az_containername, guid, self.file_name()
                )
    ...
    @classmethod
    def make_azure_signed_url(self, az_connectionstring, az_container_name, guid, file_name):
        ...
        az_signed_url = f"https://{blob_service_client.account_name}.blob.core.windows.net/{az_container_name}/{guid}/{file_name}?{account_sas_token}"
